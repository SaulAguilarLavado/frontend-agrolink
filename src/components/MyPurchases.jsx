    ...
    <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' }}>
      <Container sx={{ py: 6 }}>
        <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 3 }}>
          ...
          <Box sx={{ flexGrow: 1 }} />
          <Button onClick={() => load()} startIcon={<Replay />} sx={{ color: 'white' }}>Refrescar</Button>
        ...
        <Paper elevation={0} sx={{ p: 2, mb: 3, background: 'rgba(255,255,255,0.25)', borderRadius: 2 }}>
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ xs: 'stretch', md: 'center' }}>
            ...
            <TextField label="Email vendedor" size="small" value={sellerEmail} onChange={(e) => setSellerEmail(e.target.value)} />
            <Button variant="outlined" onClick={clearFilters}>Limpiar</Button>
            <Chip icon={<AttachMoney />} color="success" label={`Mostrando ${filtered.length}`} />
          </Stack>
        </Paper>
